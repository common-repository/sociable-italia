...
/**
 * This function makes sure Sociable-Italia is able to load the different language files from
 * the i18n subfolder of the Sociable-Italia directory
 **/
function sociable_init_locale(){
	global $sociablepluginpath;
	load_plugin_textdomain('sociable_italia', false, 'i18n');
}
add_filter('init', 'sociable_init_locale');


/**
 * @global array Contains all sites that Sociable-Italia supports, array items have 4 keys:
...
$sociable_known_sites = Array(
	...
	'Buzz' => Array(
		...
		'spriteCoordinates' => Array(127,92),
	),
...
/**
 * Returns the Sociable-Italia links list.
 *
 * @param array $display optional list of links to return in HTML
 * @global $sociable_known_sites array the list of sites that Sociable-Italia uses
...
function sociable_html($display=array()) {
	...
	/**
	 * Make it possible for other plugins or themes to add buttons to Sociable-Italia
	 */
	...
	// If a path is specified where Sociable-Italia should find its images, use that, otherwise, 
	// set the image path to the images subdirectory of the Sociable-Italia plugin.
...
if (is_array($sociable_contitionals) and in_array(true, $sociable_contitionals)) {
	...
	/**
	 * Loop through the settings and check whether Sociable-Italia should be outputted.
	 */
...
/**
 * Add the Sociable-Italia menu to the Settings menu
...
function sociable_restore_config($force=false) {
	...
	if ($force or !is_string(get_option('sociable_tagline')))
		update_option('sociable_tagline', "<strong>" . __("Share and Enjoy:", 'sociable_italia') . "</strong>");
...
/**
 * Make sure the required javascript files are loaded in the Sociable-Italia backend, and that they are only
 * loaded in the Sociable-Italia settings page, and nowhere else.
 */
...
/**
 * Make sure the required stylesheet is loaded in the Sociable-Italia backend, and that it is only
 * loaded in the Sociable-Italia settings page, and nowhere else.
 */
 ...
 * in Sociable-Italia. For all other sites this has so far been refused.
 */
...
/**
 * If the user has the (default) setting of using the Sociable-Italia CSS, load it.
 */
...
/**
 * Displays a checkbox that allows users to disable Sociable-Italia on a
...
function sociable_meta() {
	...
	?>
	<input type="checkbox" id="sociableoff" name="sociableoff" <?php checked($sociableoff); ?>/> <label for="sociableoff"><?php _e('Sociable-Italia disabled?','sociable_italia') ?></label>
	<?php
}
...
function sociable_meta_box() {
	add_meta_box('sociable_italia','Sociable-Italia','sociable_meta','post','side');
	add_meta_box('sociable_italia','Sociable-Italia','sociable_meta','page','side');
}
...
/**
 * Displays the Sociable-Italia admin menu, first section (re)stores the settings.
 */
function sociable_submenu() {
	...
	if (isset($_REQUEST['restore']) && $_REQUEST['restore']) {
		...
		sociable_message(__("Restored all settings to defaults.", 'sociable_italia'));
		...
		sociable_message(__("Saved changes.", 'sociable_italia'));
	}
...
<div class="wrap">
	<?php screen_icon(); ?>
	<h2><?php _e("Sociable-Italia Options", 'sociable_italia'); ?></h2>
	<table class="form-table">
	<tr>
		<th>
			<?php _e("Sites", "sociable"); ?>:<br/>
			<small><?php _e("Check the sites you want to appear on your site. Drag and drop sites to reorder them.", 'sociable_italia'); ?></small>
		</th>
		<td>
	...
	<tr>
		...
		<td>
			<?php _e("Change the text displayed in front of the icons below. For complete customization, copy the contents of <em>sociable.css</em> in the Sociable-Italia plugin directory to your theme's <em>style.css</em> and disable the use of the sociable stylesheet below.", 'sociable_italia'); ?><br/>
	...
	<tr>
		...
		<td>
			<?php _e("The icons appear at the end of each blog post, and posts may show on many different types of pages. Depending on your theme and audience, it may be tacky to display icons on all types of pages.", 'sociable_italia'); ?><br/>
			<br/>
			<?php
			/**
			 * Load conditions under which Sociable-Italia displays
			 */
			$conditionals 	= get_option('sociable_conditionals');
			?>
			<input type="checkbox" name="conditionals[is_home]"<?php checked($conditionals['is_home']); ?> /> <?php _e("Front page of the blog", 'sociable_italia'); ?><br/>
			<input type="checkbox" name="conditionals[is_single]"<?php checked($conditionals['is_single']); ?> /> <?php _e("Individual blog posts", 'sociable_italia'); ?><br/>
			<input type="checkbox" name="conditionals[is_page]"<?php checked($conditionals['is_page']); ?> /> <?php _e('Individual WordPress "Pages"', 'sociable_italia'); ?><br/>
			<input type="checkbox" name="conditionals[is_category]"<?php checked($conditionals['is_category']); ?> /> <?php _e("Category archives", 'sociable_italia'); ?><br/>
			<input type="checkbox" name="conditionals[is_tag]"<?php checked($conditionals['is_tag']); ?> /> <?php _e("Tag listings", 'sociable_italia'); ?><br/>
			<input type="checkbox" name="conditionals[is_date]"<?php checked($conditionals['is_date']); ?> /> <?php _e("Date-based archives", 'sociable_italia'); ?><br/>
			<input type="checkbox" name="conditionals[is_author]"<?php checked($conditionals['is_author']); ?> /> <?php _e("Author archives", 'sociable_italia'); ?><br/>
			<input type="checkbox" name="conditionals[is_search]"<?php checked($conditionals['is_search']); ?> /> <?php _e("Search results", 'sociable_italia'); ?><br/>
			<input type="checkbox" name="conditionals[is_feed]"<?php checked($conditionals['is_feed']); ?> /> <?php _e("RSS feed items", 'sociable_italia'); ?><br/>
		</td>
	</tr>
	<tr>
		<th scope="row" valign="top">
	...
	<tr>
		...
		<td>
			<?php _e("Sociable-Italia comes with a nice set of images, if you want to replace those with your own, enter the URL where you've put them in here, and make sure they have the same name as the ones that come with Sociable-Italia.", 'sociable_italia'); ?><br/>
	...
	<tr>
		...
		<td>
			<?php _e("You can choose to automatically have the links posted to certain sites shortened via awe.sm and encoded with the channel info and your API Key.", 'sociable_italia'); ?><br/>
			...
			<?php _e("awe.sm API Key:", 'sociable_italia'); ?> <input size="65" type="text" name="awesmapikey" value="<?php echo get_option('sociable_awesmapikey'); ?>" />
		</td>
	</tr>
	<tr>
		<th scope="row" valign="top">
	...
	<tr>
		<td>&nbsp;</td>
		<td>
			<span class="submit"><input name="save" value="<?php _e("Save Changes", 'sociable_italia'); ?>" type="submit" /></span>
			<span class="submit"><input name="restore" value="<?php _e("Restore Built-in Defaults", 'sociable_italia'); ?>" type="submit"/></span>
		</td>
	</tr>
</table>

<h2><?php _e('Like this plugin?','sociable_italia'); ?></h2>
<p><?php _e('Why not do any of the following:','sociable_italia'); ?></p>
<ul class="sociablemenu">
	<li><?php _e('Link to it so other folks can find out about it.','sociable_italia'); ?></li>
	<li><?php _e('<a href="http://wordpress.org/extend/plugins/sociable/">Give it a good rating</a> on WordPress.org.','sociable_italia'); ?></li>
</ul>
<h2><?php _e('Need support?','sociable_italia'); ?></h2>
<p><?php _e('If you have any problems or good ideas, please talk about them in the <a href="http://wordpress.org/tags/sociable">Support forums</a>.', 'sociable_italia'); ?></p>

<h2><?php _e('Credits','sociable_italia'); ?></h2>
<p><?php _e('<a href="http://blogplay.com/plugin/">Sociable-Italia</a> was originally developed by <a href="http://push.cx/">Peter Harkins</a> and was been maintained by <a href="http://yoast.com/">Joost de Valk</a> since the beginning of 2008. On September of 2009, the new home of Sociable-Italia is <a href="http://blogplay.com">BlogPlay.com</a>. It\'s released under the GNU GPL version 2.','Sociable-Italia'); ?></p>
...
/**
 * Add an icon for the Sociable-Italia plugin's settings page to the dropdown for Ozh's admin dropdown menu
 */
function sociable_add_ozh_adminmenu_icon( $hook ) {
	...
	if ($hook == 'Sociable-Italia') return $sociableicon;
	return $hook;
}
